...
- [ ] **Error tracking is configured**: Recommended: Sentry (free tier covers most prototypes). Install with `npm install @sentry/nextjs` and follow their setup wizard. Takes 15 minutes and is worth it
...
- [ ] **Analytics are installed**: Recommended for PM-builders: **Posthog** (open source, free tier, great product analytics) or **Plausible** (privacy-friendly, simple). Google Analytics works but is overkill and complex
